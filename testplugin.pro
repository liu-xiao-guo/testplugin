...
UBUNTU_TRANSLATION_DOMAIN="testplugin.liu-xiao-guo"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/testplugin/tests/autopilot/run
aptest.depends  = sub-testplugin sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/testplugin/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-testplugin sub-backend-App
